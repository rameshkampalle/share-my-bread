...
files.set('SMB-AGT-001-Assistant.json', workflow(
  ...
  [
    ...
    code('60000000-0000-4000-8000-000000000007', 'Validate Agent Response', 260, 0,
      `const raw = String($json.output ?? '').trim();\nconst start = raw.indexOf('{');\nconst end = raw.lastIndexOf('}');\nconst jsonText = start >= 0 && end >= start ? raw.slice(start, end + 1) : raw;\nlet value;\ntry { value = JSON.parse(jsonText); } catch { value = { responseType: 'ERROR', message: 'The assistant returned an invalid response. Please try again.', requiresConfirmation: false }; }\nconst allowed = ['ANSWER','CLARIFICATION','CART_PROPOSAL','NO_MATCH','REFUSAL','ERROR'];\nif (!allowed.includes(value.responseType)) value.responseType = 'ERROR';\nvalue.correlationId = $('Normalize Agent Request').first().json.correlationId;\nvalue.requiresConfirmation = value.responseType === 'CART_PROPOSAL';\nif (value.requiresConfirmation && !value.proposal) { value.responseType = 'ERROR'; value.requiresConfirmation = false; value.message = 'A valid proposal could not be created.'; }\nreturn [{ json: value }];`),
